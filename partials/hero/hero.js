export default class HeroPartial {
  ...
  init() {
    const buttonContainer =
        document.querySelector(`.${ this.importanceButtonContainerClass }`);

    // Create the buttons
    this.importanceValues.forEach((importanceValue) => {
      buttonContainer.innerHTML +=
          `<button class="${ this.importanceButtonClass }"
              data-importance="${ importanceValue.value }">
            ${ importanceValue.name }
          </button>`;
    });

    // Add click events
    document.querySelectorAll(`.${ this.importanceButtonClass }`)
      .forEach((btn) => {
        btn.addEventListener('click', (clickEvent) => {
          const importance =
            clickEvent.target.getAttribute(this.importanceAttr);
